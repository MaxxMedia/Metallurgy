...
export function PostCard({
  ...
  const h1Class =
    variant === "one" && TitleTag === "h1"
      ? onDarkHero
        ? "text-xl font-bold leading-snug sm:text-2xl md:text-[1.625rem] lg:text-[1.75rem] lg:leading-[1.25] xl:text-[2rem]"
  ...
  if (variant === "one") {
    shell = onDarkHero
      ? "fpg-card-style style-one flex w-full max-w-[650px] flex-col items-start gap-4 border-0 border-b-0 pt-0 pb-2 shadow-none sm:pb-4"
  ...
  if (variant === "floating") {
    const floatingImgClass =
      "attachment-medium_large size-medium_large wp-post-image h-full min-h-full w-full object-cover";

    return (
      <div className={`${shell} ${className}`.trim()}>
        <div className="fpg-post-thumb absolute inset-0 h-full w-full overflow-hidden">
          <Link href={post.url} className="image-link block h-full w-full">
            <img loading="lazy" decoding="async" src={thumb} className={floatingImgClass} alt="" />
          </Link>
          <div className="thumb-overlay" aria-hidden />
        </div>
        <div className="fpg-post-content z-[2]">
